...
export function createEventBlock(options: EventBlockOptions): void {
  ...
  /* `forBlock`, not `javascriptGenerator[id]`. Both work in Blockly 10,
     but the second is the pre-v10 form: Blockly logs a deprecation for it
     and drops the fallback in v11, which would have taken every event
     block in DisFuse with it — silently, since a missing generator only
     throws when that block is actually compiled. */
